...
func _ready() -> void:
    ...
    var npc: Area2D = village.get_node("NPC")
    player.global_position = npc.global_position + Vector2(0, 100)
    ...
    assert(player.global_position.y <= npc.global_position.y, "Player should be able to move up to the NPC's row")
